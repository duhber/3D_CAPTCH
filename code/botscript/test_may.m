...
%             texfile='../OpenGLobjloader/octexture/zebra.jpeg';
...
%             for i=1:qy-1
%                 yy=wind*i;
%                 hstrip=Ip(yy-5:min(yy+6,H),:,:);
%                 hstrip=imfilter(hstrip,mask);
%                 Ip(yy:yy+1,:,:)=hstrip(6:7,:,:);
%             end
%             
%             for j=1:qx-1
%                 xx=wind*j;
%                 vstrip=Ip(:,xx-5:min(xx+6,W),:);
%                 vstrip=imfilter(vstrip,mask);
%                 Ip(:,xx:xx+1,:)=vstrip(:,6:7,:);
%             end
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%             dithPatch=orderedDith(double(dithPatch),6);
%             I2=double(I2);
            Ip=Ip./max(Ip(:));
%             I2=I2./max(I2(:));
...
%            
            I2=Ip+I2dith;
%               I2=(I2-min(I2(:)))/(max(I2(:))-min(I2(:)))*255;
%              I2=uint8(I2);
